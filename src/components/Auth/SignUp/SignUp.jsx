  ...
  const handleEmailSignUp = async (e) => {
    ...
    try {
      const { data, error } = await authClient.signUp.email({
        ...
        email: email,
        password: password,
        image: image || undefined, // Fallback to undefined if left empty string
        callbackURL: "/dashboard", // Update this target path to your post-auth page
      });

      if (error) {
        setAuthError(error.message || "An error occurred during account registration.");
      } else if (data) {
        // Redirection behavior handles automatically via callbackUrl, but fallback redirect logic here if needed
        router.push("/home");
      }
    } catch (err) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      await authClient.signIn.social({
        provider: "google",
        callbackURL: "/home", // Redirect destination once OAuth lifecycle completes
      });
    } catch (err) {
          ...
          <form className="space-y-4" onSubmit={handleEmailSignUp}>
            ...
            <div className="bg-white/40 border border-gray-100/50 rounded-xl p-3 space-y-1.5 text-[11px] font-normal text-gray-500">
              <div className="flex items-center gap-2">
                <div className={`w-1.5 h-1.5 rounded-full ${/[A-Z]/.test(password) ? "bg-[#4D7C5D]" : "bg-gray-300"}`} /> 
              ...
              <div className="flex items-center gap-2">
                <div className={`w-1.5 h-1.5 rounded-full ${password.length >= 8 ? "bg-[#4D7C5D]" : "bg-gray-300"}`} /> 
